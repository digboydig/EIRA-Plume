...
with tab2:
    ...
    **Parameters:** $Q=2000 \\text{ g/s}$, $h_s=120 \\text{ m}$, $\\Delta h=10 \\text{ m} \\implies H=130 \\text{ m}$, $U=15 \\text{ m/s}$, Stability Class **D** (Neutral).
    ...
    # (a) Concentration on plume centre-line at z = H (centerline)
    ...
    with colQ1_1:
        st.metric(label="(a) $C(800, 0, H)$", value=f"{C_plume_center_ug_m3:,.1f} μg/m^3", delta="Target: 5311 μg/m^3")
    with colQ1_2:
        st.metric(label="(b) $C(800, 0, 0)$", value=f"{C_ground_ug_m3:,.1f} μg/m^3", delta="Target: 257 μg/m^3")
    with colQ1_3:
        st.metric(label="(c) $x_{max}$", value=f"{max_x_loc:,.1f} m", delta="Target: 1542.3 m")
    with colQ1_4:
        st.metric(label="(d) $C_{max}$", value=f"{max_C_ug_m3:,.1f} μg/m^3", delta="Target: 1051 μg/m^3")
    ...
    **Emission Rate:** $Q = 1 \\text{ m}^3/\\text{s} \\times 1.64 \\text{ kg/m}^3 = 1640 \\text{ g/s}$.
    """)
    ...
    with colQ2_1:
        st.metric(label="(a) $C(1500, 500, 0)$", value=f"{C_off_center_ug_m3:,.1f} μg/m^3", delta="Target: 43 μg/m^3")
    with colQ2_2:
        st.metric(label="(b) $C(1500, 0, 0)$", value=f"{C_on_center_ug_m3:,.1f} μg/m^3", delta="Target: 1537 μg/m^3")
    ...
    **Stability:** Class B.
    ...
    st.metric(label="Ground Level Concentration ($C(2500, 0, 0)$)", value=f"{C_Q3_ug_m3:,.1f} μg/m^3", delta="Target: 1400 μg/m^3")
    ...
    **Stability:** Class F.
    **Gross Screening:** $C_{wc} = \\dfrac{10^9 Q}{U H_{wc} W_{wc}}$ with $H_{wc}=50m$, $W_{wc}=0.1x$.
    """)
    ...
    with colQ4_1:
        st.metric(label="Gross Screening Concentration", value=f"{C_screening_ug_m3:,.2f} μg/m^3", delta="Target: 25 μg/m^3")
    with colQ4_2:
        st.metric(label="Gaussian Plume Concentration (Stability F)", value=f"{C_gaussian_ug_m3:,.2f} μg/m^3", delta="Target: 0.56 μg/m^3")
